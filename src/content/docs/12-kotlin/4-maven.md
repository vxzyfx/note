...
- `xmlns` 和 `xsi:schemaLocation` 定义了 POM 文件的命名空间和 XML Schema 位置。
- `<modelVersion>` 定义了 POM 模型的版本，这里使用的是 `4.0.0`。
...
- `plugin`：每个插件的定义，包括 `groupId` , `artifactId` , `version` 和 `configuration`。这里配置了 `maven-compiler-plugin`，指定编译的 Java 版本为 1.8。
...
- `properties`：定义项目的属性，可以在 POM 文件的其他部分引用。
- `maven.compiler.source` 和 `maven.compiler.target`：指定 Java 编译器的源版本和目标版本。
