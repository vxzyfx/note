...
1. **切面 (Aspect):** 切面是 AOP 的核心概念，它封装了横切关注点的行为。一个切面通常包括一个或多个切入点和相应的增强（advice）。
...
1. **提高代码模块化：** 通过将横切关注点独立出来，AOP 使得业务逻辑代码更清晰、更聚焦，从而提高了代码的可读性和可维护性。
...
2. **类加载时织入 (Load-time Weaving):** 在类被加载到 JVM 时，织入过程发生，此时增强逻辑被注入到目标类中。
...
java 中的 AOP 时通过动态代理实现

动态代理的实现:

- JDK 动态代理: 目标有接口时才可以使用, 生成接口实现类进行代理
- cglib 动态代理: 目标没有接口, 通过继承目标, 实现代理

`AspectJ` 是一个 AOP 框架, Spring 借用类 AspectJ 的注解
...
`@EnableAspectJAutoProxy` 开启 AOP 动态代理
