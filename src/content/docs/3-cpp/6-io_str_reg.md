...
C++ 标准库 `std::regex` 没有提供 ECMAScript 风格命名捕获组和 `match["name"]` 访问方式。需要命名捕获时，应使用第三方正则库，或在标准库示例中使用位置索引。
...
int main() {
    ...
    std::regex re(R"((\w+), and I am (\d+) years old.)");

    std::smatch match;
    if (std::regex_search(str, match, re)) {
        std::cout << "Name: " << match[1].str() << std::endl;
        std::cout << "Age: " << match[2].str() << std::endl;
    } else {
        std::cout << "No match found." << std::endl;
    }
