---
title: linux 工具
description: tcpdump 与 iperf3 网络诊断工具的参数和使用注意事项。
---
<!-- cspell:words tcpdump iperf Mbit logfile bidir nodelay Nagle pcap Mbps -->

## tcpdump

> 操作提醒：抓包可能包含 IP、域名、Cookie、Token、明文协议内容等敏感信息。保存 pcap 文件前先确认授权范围，分享前做脱敏处理。

tcpdump 虽然名称带有 tcp, 但是它不只是用于 tcp, 而是支持所有协议.
...
> 表示主机地址, host 192.168.1.1 表示抓取 192.168.1.1 的报文

1. net

> net 192.168.1.0/24 表示抓取 192.168.1.0/24 网段的报文

1. port

> port 80 表示抓取 80 端口的报文

1. dst

> dst 192.168.1.1 表示抓取 192.168.1.1 作为目的地的报文

1. src

> src 192.168.1.1 表示抓取 192.168.1.1 作为源地的报文

1. ip, ip6

> ip 表示抓取所有 ip 报文, ip6 表示抓取所有 ipv6 报文
...
> and 表示与操作, 例如 host 192.168.1.1 and port 80 表示抓取 192.168.1.1 作为源地, 80 端口的报文

1. or

> or 表示或操作, 例如 host 192.168.1.1 or port 80 表示抓取 192.168.1.1 作为源地, 80 端口的报文或者 192.168.1.1 作为目的地, 80 端口的报文

1. !

> ! 表示非操作, 例如 !host 192.168.1.1 表示抓取不是 192.168.1.1 的报文
...
> 操作提醒：`iperf3 -s` 会监听端口，客户端压测会占用带宽并可能触发告警。只在授权网络和维护窗口内测试，避免对生产链路造成拥塞。
...
1. [tcpdump man page](https://www.tcpdump.org/manpages/tcpdump.1.html)（访问日期：2026-05-31）
1. [iperf3 manual page](https://software.es.net/iperf/invoking.html)（访问日期：2026-05-31）
