---
title: neovim 笔记
description: Neovim 官方文档入口、界面概念和选项速查。
---
<!-- cspell:words Nvim Vimscript nvim keepascii CDPATH cindent unnamedplus closeoff linematch lastline equalalways eventignore nvimrc exrc charconvert foldmethod foldtext foldexpr tcqj vimgrep blinkon blinkoff VIMRUNTIME iconstring lmap nosplit smagic snomagic langmap chistory breakat showmatch mkspell modeline modelines modelineexpr setpos runtimepath IPLPPPQPP TPHPLI Itpplpipbp nroff thegf scrollbind SHNHH Unhsh curdir tabpages winsize mksession shada shellxquote shiftwidth ignorecase uselast followic timeoutlen titlestring ttimeoutlen mkview wildcharm wildchar tagfile cmdline redrawdebug -->

## 官方文档链接

1. [Nvim API via RPC](https://neovim.io/doc/user/api.html)
1. [Nvim UI protocol](https://neovim.io/doc/user/api-ui-events.html)
1. [Nvim Lua guide](https://neovim.io/doc/user/lua-guide.html)
1. [Lua API](https://neovim.io/doc/user/lua.html)
1. [Lua reference manual](https://neovim.io/doc/user/luaref.html)
1. [Luv reference manual](https://neovim.io/doc/user/luvref.html)
1. [Event handlers](https://neovim.io/doc/user/autocmd.html)
1. [Spawn and control multiple processes](https://neovim.io/doc/user/job_control.html)
1. [Nvim asynchronous IO](https://neovim.io/doc/user/channel.html)
1. [Vimscript reference](https://neovim.io/doc/user/vimeval.html)
1. [Vimscript functions](https://neovim.io/doc/user/vimfn.html)
1. [Nvim remote plugins](https://neovim.io/doc/user/remote_plugin.html)
1. [Health checking](https://neovim.io/doc/user/health.html)
1. [Language Server Protocol](https://neovim.io/doc/user/lsp.html)
1. [Diagnostic framework](https://neovim.io/doc/user/diagnostic.html)
1. [Neovim options](https://neovim.io/doc/user/options.html)
...
- `se[t] {option}&vi` 设置为 vi 默认值
- `se[t] {option}&vim` 设置为 vim 默认值
...
|`ambiwidth` `ambw`|global|string|single|针对 East Asian Ambiguous Width 字符显示宽度控制的选项(single: 符按 1 列处理. double: 按 2 列处理)|
...
|`background` `bg`|global|string|dark|告诉编辑器当前所使用的终端或 GUI 背景色是浅色（light）还是深色（dark）|
...
|`casemap` `cmp`|global|string|internal,keepascii|控制大小写转换行为的选项，主要影响 ~（切换大小写）命令和 gU/g~ /gu 系列命令在处理特定字符时的行为|
|`cdhome` `cdh`|global|boolean|off|当 on 时，没有参数的 `:cd`、`:tcd` 和 `:lcd` 会将当前工作目录更改为 `$HOME` 目录|
...
|`channel`|buffer|number|0|不是 :set 的配置项, 用于实现 Neovim 的异步通信机制|
...
|`cinkeys` `cink`|buffer|string|"0{,0},0),0],:,0#,!^F,o,O,e"|控制何时触发重新缩进的键位规则的选项，主要配合 cindent 一起工作|
...
|`cmdheight` `ch`|global ot tab|number|1|设置命令行区域的高度|
|`cmdwinheight` `cwh`|global|number|7|设置命令行窗口（Command-line window）高度的选项，控制使用 q:、q/、q? 打开命令历史编辑窗口时的默认行数|
|`colorcolumn` `cc`|window|string|""|高亮特定列数位置的视觉辅助选项，常用于提醒开发者代码行是否超出某个字符宽度限制|
|`columns` `co`|global|number|80 或终端宽度|当前编辑器窗口的列宽|
|`comments` `com`|buffer|string|-|影响自动注释续行（比如回车时是否自动补上 // 或 \*）|
|`commentstring` `cms`|buffer|string|""|定义当前文件类型注释格式的字符串选项|
|`complete` `cpt`|buffer|string|".,w,b,u,t"|配置插入模式补全行为的选项|
|`completefunc` `cfu`|buffer|string|""|插入模式补全的函数接口|
|`completefuzzycollect` `cfc`|global|string|""|用于模糊匹配（fuzzy matching）补全候选项的收集阶段|
...
|`completeslash` `csl`|buffer|string|""|仅在 Windows 下可修改, 在 Windows 下设置路径补全，覆盖 `shellslash`|
...
|`cpoptions` `cpo`|global|string|"aABceFs\_"|vi 兼容选项|
...
|`eventignorewin` `eiw`|window|string|""|类似于“eventignore”，但适用于特定窗口及其 buffers|
|`expandtab` `et`|buffer|boolean|off|使用适当数量的空格插入\<Tab\>|
|`exrc` `ex`|global|boolean|off|启用项目本地配置。Nvim 将执行任何 .nvim.lua，.nvimrc 或 .exrc 文件|
|`fileencoding` `fenc`|buffer|string|""|当前缓冲区的文件内容编码。转换是通过 iconv（）或使用 'charconvert' 指定|
...
|`filetype` `ft`|buffer|string|""|文件类型, 设置此选项后，将触发 file type 自动命令事件|
...
|`icon`|global|boolean|off|打开时，窗口的图标文本将设置为 iconstring|
...
|`lhistory` `lhi`|window|number|10|与 'chistory' 类似，但对于与 window|
|`linebreak` `lbr`|window|boolean|off|如果打开，Vim 将在 'breakat' 中的字符处换行|
|`lines`|global|number|24 或终端高度|Vim 窗口的行数|
|`linespace` `lsp`|global|number|0|GUI 中字符之间插入的像素线数|
|`lisp`|buffer|boolean|off|Lisp 模式：当 \<Enter\> 在插入模式下键入时，将缩进设置为 Lisp 标准的下一行|
...
|`modelineexpr` `mle`|global|boolean|off|打开时，允许在 modeline。检查选项是否受到'modelineexpr'|
...
|`showmode` `smd`|global|boolean|on|显示 Vim 模式|
...
|`tagrelative` `tr`|global|boolean|on|在另一个目录中使用标签文件则该目录中的文件名 tags 文件相对于 tags 文件所在的目录|
